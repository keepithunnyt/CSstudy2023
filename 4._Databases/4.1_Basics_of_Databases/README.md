...
- [4.1.6 관계](#416-관계)
  ...
  - [N:M 관계](#nm-관계)
- [4.1.7 키](#417-키)
  - [외래키(Foreign Key)](#외래키foreign-key)
...
# 4.1.6 관계
- 관계화살표로 관계를 표시(화살표의 종류는 검색 이미지 참고)
- 관계 종류
...
## N:M 관계
- N:M 관계는 테이블 두 개를 직접 연결해서 구축하지는 않음.
...
# 4.1.7 키
- 기본키, 외래키, 후보키, 슈퍼키, 대체키 등이 있음.
  - 슈퍼키는 유일성이 있음. 후보키는 유일성 + 최소성도 갖춤.
  - 대채키는 후보키 중 기본키로 선택되지 못한 키.
    - 유일성 : 중복되는 값이 없음
...
### 자연키
- 중복되지 않는 값들이 자연스럽게 골라진 키
- 언젠가 변경될 수 있다.

### 인조키
- 인위적으로 ID 등을 부여해서 고유식별자를 생성한 키
- 오라클 : sequence, MySQL은 auto increment 등으로 설정
- 자연키와 대조적으로 변하지 않는 값이다.
- 보통 기본키는 인조키로 설정함


## 외래키(Foreign Key)
- 기본키를 그대로 참조하는 값으로, 개체와의 관계를 식별하는데 사용
- 중복되어도 괜찮다.
...
## 대체키(Alternate Key)
- 후보키가 두 개 이상일 경우 어느 하나를 기본키로 지정하고, 남은 후보를 의미
